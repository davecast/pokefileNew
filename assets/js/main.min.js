...
},{"./components/home/":8}],3:[function(require,module,exports){
var APPS = [];
...
var _POKEMONES = require('./_database');
...
APP.constant('api', {url: API,sprite:SPRITE,pokeBD:_POKEMONES,});
...
},{"./_database":1,"./app.modules":3}],6:[function(require,module,exports){
require('./shared/dcdialog/');
...
},{"./shared/dcdialog/":13,"./shared/filters/":19,"./shared/footer/":22,"./shared/header/":26,"./shared/libs/":30,"./shared/search/":34}],7:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.controller('homeController', ['$scope', '$timeout', '$interval', 'homeService', '$rootScope', 'dcDialogService', function ($scope, $timeout, $interval, homeService,$rootScope, dcDialogService){
    $scope.home = {};
    $scope.allPage = [];
    $scope.pokeonFound = [];
    $scope.currentPage = 1;
    ...
    function hexToRgb(hex) {
        ...
        var rgbC = parseInt(result[1], 16) + ',' + parseInt(result[2], 16) + ',' + parseInt(result[3], 16);
        return result ? rgbC : null;
    }
    
    $scope.home.setColor =  function (type) {
        
        var types = {    
            normal : hexToRgb('#ececec'),
            fire : hexToRgb('#f08030'),
            fighting : hexToRgb('#f7924d'),
            water : hexToRgb('#6890f0'),
            flying : hexToRgb('#c3f3f4'),
            grass : hexToRgb('#8bbb40'),
            poison : hexToRgb('#a040a0'),
            electric : hexToRgb('#f8d030'),
            ground : hexToRgb('#635d21'),
            psychic : hexToRgb('#f85888'),
            rock : hexToRgb('#b8a038'),
            ice : hexToRgb('#98d8d8'),
            bug : hexToRgb('#a8b820'),
            dragon : hexToRgb('#f8254f'),
            ghost : hexToRgb('#705898'),
            dark : hexToRgb('#705848'),
            steel : hexToRgb('#b8b8d0'),
            fairy : hexToRgb('#ee99ac')
        };
    ...
    $scope.home.openDialog = function (obj) {
        $rootScope.infoDialog = false;
        dcDialogService.open(obj).then(function(data){
            if(data){
                $rootScope.infoDialog = true;
                $rootScope.infoPoke = data;
                var colorPrimario = $scope.home.setColor($rootScope.infoPoke.p.types[0].type.name);
                var colorSecundario = ($rootScope.infoPoke.p.types.length > 1) ? $scope.home.setColor($rootScope.infoPoke.p.types[1].type.name) : $scope.home.setColor($rootScope.infoPoke.p.types[0].type.name);
                $('.dc-dialog').css({
                    background: 'linear-gradient(45deg, rgb('+colorPrimario+') 50%, rgb('+colorSecundario+') 50%)'
                });
                var nums = [];
                for (var i = 0; i < $rootScope.infoPoke.p.stats.length; i++ ) {
                    nums.push($rootScope.infoPoke.p.stats[i].base_stat);
                }
                $rootScope.statMax = getMaxOfArray(nums);
                console.log(data);
            }
        });    
    };
        ...
        if($scope.pageOn){
            ...
            homeService.allData(url).then(function(data){
                if (data){
                    ...
                    timer = $timeout(function() {$rootScope.loadWidth = 'false'; $timeout.cancel( timer );},2000);
                    $scope.pageOn = true;
                    $scope.allPokemons = data;
                    $scope.next = data.data.next;
                    $scope.prev = data.data.previous;
...
},{"../../app.settings":5}],8:[function(require,module,exports){
require('./service');
require('./controller');
},{"./controller":7,"./service":9}],9:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5}],10:[function(require,module,exports){
require('./app.routes');
require('./app.settings');
require('./app.shared');
require('./app.components');
},{"./app.components":2,"./app.routes":4,"./app.settings":5,"./app.shared":6}],11:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.controller('dcDialogController', ['$scope', 'dcDialogService',
function ($scope, dcDialogService) {
    
    $scope.dialog = {};
    $scope.dialog.close = dcDialogService.close;
    
}]);

},{"../../app.settings":5}],12:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.directive('dcDialog', function(){
	...
	return {
		...
		controller: 'dcDialogController',
		link:  link
	};

});
},{"../../app.settings":5,"./view.html":15}],13:[function(require,module,exports){
require('./service');
require('./controller');
require('./directive');
},{"./controller":11,"./directive":12,"./service":14}],14:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.factory('dcDialogService', ['$location', '$http', '$q', 'api', '$rootScope',
function ($location, $http, $q, api, $rootScope){
  
	function open(obj) {
		var id,str;
		$('#'+obj.id).addClass('dc-dialog-open');
		$('#'+obj.id).removeClass('dc-dialog-close');
		$('body').addClass('dc-body-dialog');
		var deferred = $q.defer();
		
		view(obj.url).then(function(data){
	...
	function close(id) {
		...
		$('#'+id).removeClass('dc-dialog-open');
		$('#'+id).addClass('dc-dialog-close');
		$('body').removeClass('dc-body-dialog');
		$rootScope.infoDialog = false;
	}
...
},{"../../app.settings":5}],15:[function(require,module,exports){
module.exports = "<div data-ng-switch=infoDialog class=dc-dialog-switch-container><div data-ng-switch-when=true class=dc-dialog-switch><dc-dialog-content class=dc-dialog-struct><dc-dialog-title class=\"dc-dialog-struct-title {{ (infoPoke.p.types[1].type.name) ? infoPoke.p.types[1].type.name : infoPoke.p.types[0].type.name }}\">{{infoPoke.p.name}}</dc-dialog-title><dc-dialog-body class=dc-dialog-struct-body><div class=dc-dialog-struct-body-picture><figure><img ng-src=\"{{infoPoke.sprite}}{{infoPoke.p.name | fixImg}}.gif\" alt={{infoPoke.p.name}}></figure></div><div class=dc-dialog-struct-body-stats><div class=dc-dialog-struct-body-stats-id># {{infoPoke.id}}</div><div class=\"dc-content-item-body-info-types dc-dialog-types\"><div class=\"types type-{{t.type.name}}\" data-ng-repeat=\"t in infoPoke.p.types\"></div></div><div class=dc-dialog-struct-body-stats-bar><div class=dc-dialog-struct-body-stats-bar-list ng-repeat=\"stat in infoPoke.p.stats\"><div class=dc-dialog-struct-body-stats-bar-list-name>{{stat.stat.name}}</div><div class=dc-dialog-struct-body-stats-bar-list-stat><div class=\"bar-charge {{ (infoPoke.p.types[1].type.name) ? infoPoke.p.types[1].type.name : infoPoke.p.types[0].type.name }}\" style=\"width:0%; width: {{stat.base_stat | barCharge}}%\"><div class=bar-number>{{stat.base_stat}}</div></div></div></div></div></div></dc-dialog-body></dc-dialog-content></div><div data-ng-switch-when=false class=dc-dialog-switch><dc-dialog-content class=\"dc-dialog-struct loading\">loading</dc-dialog-content></div></div><dc-dialog-closed data-ng-click=\"dialog.close('pokemonInvidual')\" class=dc-dialog-closed><i class=\"mdi mdi-close\"></i></dc-dialog-closed>";

},{}],16:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.filter('barCharge', ['$filter', '$rootScope', function($filter, $rootScope) {
	return function(input) {
		
		var number = ((input*0.80)/$rootScope.statMax)*100;
		
		return number;
	};
}]);
},{"../../app.settings":5}],17:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.filter('fixImg', ['$filter', function($filter) {
	return function(input) {
		var name;
		...
		var verific = nameWrong[input];
		
		if (verific === undefined) {
			name = input;
		} else {
			name = verific;
		}
...
},{"../../app.settings":5}],18:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5}],19:[function(require,module,exports){
require('./idfilter');
require('./fiximg');
require('./barcharge');
},{"./barcharge":16,"./fiximg":17,"./idfilter":18}],20:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5}],21:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5,"./view.html":23}],22:[function(require,module,exports){
require('./controller');
require('./directive');
},{"./controller":20,"./directive":21}],23:[function(require,module,exports){
...
},{"../../app.settings":5}],25:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5,"./view.html":28}],26:[function(require,module,exports){
arguments[4][13][0].apply(exports,arguments)
},{"./controller":24,"./directive":25,"./service":27}],27:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5}],28:[function(require,module,exports){
module.exports = "<div class=\"header dc-flex dc-center-center dc-flex-column\"><div><h1>HEADER</h1></div><div class=\"dc-h1 dc-flex dc-center-center dc-flex-column\"><h2>POKEFILE 2016</h2><div class=dc-h1-title>Colores que se usaran en la pagina.</div><div class=dc-h1-colors><div class=\"col1 divc\"></div><div class=\"col2 divc\"></div><div class=\"col3 divc\"></div><div class=\"col4 divc\"></div><div class=\"col5 divc\"></div><div class=\"col6 divc\"></div><div class=\"col7 divc\"></div></div><div class=dc-h1-title>Baja para ver mas!!</div></div></div>";
...
},{"../../app.settings":5}],30:[function(require,module,exports){
require('./fssrc');
require('./loader');

},{"./fssrc":29,"./loader":31}],31:[function(require,module,exports){
var _APP = require('../../app.settings');
...
},{"../../app.settings":5}],32:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.controller('searchController', ['$scope', 'searchService', 'dcDialogService', '$rootScope',
function ($scope, searchService, dcDialogService, $rootScope) {
    
    $scope.sch = {};
    $scope.sch.search = searchService.byName;
    $scope.sch.allPokemon = [];
    $scope.sch.isOpen = false;
    $scope.sch.icon = 'magnify';
    $rootScope.infoDialog = false;
    ...
    $scope.sch.setColor =  function (type) {
    ...
    $scope.sch.openDialog = function (obj) {
        $scope.sch.textContent = '';
        $scope.sch.allPokemon = [];
        $scope.sch.icon = 'magnify';
        $('#sch').removeClass('pokemon-search-act');
        $scope.sch.isOpen = false;
        dcDialogService.open(obj).then(function(data){
            if(data){
                $rootScope.infoDialog = true;
                $rootScope.infoPoke = data;
                var colorPrimario = $scope.sch.setColor($rootScope.infoPoke.p.types[0].type.name);
                var colorSecundario = ($rootScope.infoPoke.p.types.length > 1) ? $scope.sch.setColor($rootScope.infoPoke.p.types[1].type.name) : $scope.sch.setColor($rootScope.infoPoke.p.types[0].type.name);
                console.log(data);
                $('.dc-dialog').css({
                    background: 'linear-gradient(45deg, rgb('+colorPrimario+') 50%, rgb('+colorSecundario+') 50%)'
                });
    ...
    $scope.sch.actionSearch = function(){
        if ($scope.sch.isOpen === true) {
            $scope.sch.textContent = '';
            $scope.sch.allPokemon = [];
            $scope.sch.icon = 'magnify';
            $('#sch').removeClass('pokemon-search-act');
            $scope.sch.isOpen = false;
        } else if ($scope.sch.isOpen === false) {
            $scope.sch.allPokemon = searchService.all();
            $scope.sch.isOpen = true;
            $scope.sch.icon = 'close';
            $('#sch').addClass('pokemon-search-act');
        }
    };

}]);

},{"../../app.settings":5}],33:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.directive('setSearch', function(){
...
},{"../../app.settings":5,"./view.html":36}],34:[function(require,module,exports){
arguments[4][13][0].apply(exports,arguments)
},{"./controller":32,"./directive":33,"./service":35}],35:[function(require,module,exports){
var _APP = require('../../app.settings');

_APP.factory('searchService', ['$location', '$http', '$q', 'api',
function ($location, $http, $q, api){
  ...
  function byName(name){
    ...
    names = [];
    api.pokeBD.filter(function(pokemon){
      if (pokemon.name.indexOf(name) > -1) {
        
        
        var obj = {
          name: pokemon.name,
          url: pokemon.url,
          sprite: api.sprite + pokemon.name+'.gif'
        };
        names.push(obj);
      }
    });
...
},{"../../app.settings":5}],36:[function(require,module,exports){
module.exports = "<div class=pokemon-search-box><div class=pokemon-search-box-input><input type=text name=poke-finder data-ng-model=sch.textContent data-ng-init=\"sch.textContent=''\" placeholder=\"busca tu pokemon\"></div><div class=pokemon-search-box-icon data-ng-click=sch.actionSearch()><i class=\"mdi mdi-{{sch.icon}}\"><i></i></i></div></div><div class=pokemon-search-list data-ng-if=\"sch.textContent.length > 2\"><ul class=pokemon-search-list-ul><li data-ng-click=\"sch.openDialog({id:'pokemonInvidual',url:find.url})\" class=\"pokemon-search-list-ul-li animate-repeat\" data-ng-repeat=\"find in sch.allPokemon | filter:sch.textContent as results\"><div class=pokemon-search-list-ul-li-figure><figure><img ng-src=\"https://play.pokemonshowdown.com/sprites/xyani/{{find.name | fixImg}}.gif\"></figure></div><div class=pokemon-search-list-ul-li-name>{{find.name}}</div></li><li class=\"not-found animate-repeat\" data-ng-if=\"results.length == 0\"><strong>No se encontraron resultados</strong></li></ul></div>";
